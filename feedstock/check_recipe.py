...
def main():
    ...
    client = Client(
        processes=False,
        n_workers=1,
        threads_per_worker=4,    # use 8 threads (or adjust based on your CPU cores)
        memory_limit="128GB"      # allocate half your system memory
    )
                ...
                if response_mean.status_code == 200 and response_std.status_code == 200:
                    with MemoryFile(response_mean.content) as memfile_mean, MemoryFile(response_std.content) as memfile_std:
                        with memfile_mean.open() as src_mean, memfile_std.open() as src_std:
                            ...
                            #min_val = float(da_mean.min().compute())
                            #max_val = float(da_mean.max().compute())
                            #logger.info(f"Tile {i} range: {min_val:.2f}–{max_val:.2f}")
    ...
    for i, file_name in enumerate(file_names[N:M],start=N):
        ...
        #skipped_tiles = []
        report_mem("before read")
        ds = read_canopy_file(file_name, base_url,i)#,skipped_tiles)
        report_mem("after read but before write")
        ...
        if  first_write:
            encoding = {
            var: {"compressor": compressor, "chunks": (1,4500,6000)}
            for var in ds.data_vars
            }#must match other chunking
            
            ds.to_zarr(
                      store,
                      mode="w",
                      consolidated=first_write,
                      encoding=encoding,
                      zarr_version=2
                    )
            ...
            ds.to_zarr(store, mode="a", append_dim="tile_id", consolidated=first_write, zarr_version=2)
